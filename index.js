...
client.connect(() => console.log("MongoDB connected")).catch(console.dir);
...
const billingCollection = database.collection("billing");
...
//set dat u=in the req object
req.user = user;

console.log('user of the session', userId);

  next();
}
//must be used after verifyToken after middleware
...
const verifyRecruiter = async(req,res,next)=>{
  ...
  next();
    ...
    app.post('/api/billing', async(req, res) => {
      ...
      const result = await billingCollection.insertOne(newRecord);
      res.send(result);
    });

    app.get('/api/billing', logger, verifyToken, verifyAdmin, async(req, res) => {
      const cursor = billingCollection.find().sort({ createdAt: -1 });
      const result = await cursor.toArray();
      res.send(result);
    });

    app.delete('/api/billing/:id', logger, verifyToken, verifyAdmin, async(req, res) => {
      const id = req.params.id;
      const query = { _id: new ObjectId(id) };
    ...
    app.patch('/api/billing/:id', logger, verifyToken, verifyAdmin, async(req, res) => {
      const id = req.params.id;
      ...
      const result = await billingCollection.updateOne(filter, updatedDoc);
      res.send(result);
    });
    ...
    app.get('/api/settings', async(req, res) => {
      ...
      if (!settings) {
        ...
        const defaultSettings = {
          type: 'global',
          platformName: 'Talentist Velo',
          supportEmail: 'support@talentist.com',
          currency: 'USD',
          enableSignups: true,
          enableJobPosting: true,
          requireAdminApproval: false,
          maintenanceMode: false,
          metaTitle: 'Talentist Velo - Discover Your Next Challenge',
          metaDescription: 'Find jobs and hire top talent securely.',
          createdAt: new Date()
        };
        await settingsCollection.insertOne(defaultSettings);
        settings = await settingsCollection.findOne({ type: 'global' });
      }
      res.send(settings);
    });

    app.patch('/api/settings', logger, verifyToken, verifyAdmin, async(req, res) => {
      ...
      // Upsert just in case
      const result = await settingsCollection.updateOne(filter, updatedDoc, { upsert: true });
      res.send(result);
    });
    ...
    app.get('/api/billing/:email', async(req, res) => {
      ...
      // Sort by createdAt descending (-1) to show newest first
      const cursor = billingCollection.find(query).sort({ createdAt: -1 });
      const result = await cursor.toArray();
      res.send(result);
    });
  ...
  app.get('/api/applications/applicant',verifyToken,verifySeeker, async(req,res)=>{
    const query = {}; 
   if(req.query.applicantId){
    query.applicantId = req.query.applicantId;
// check whether asking for user information or someone 
  ...
  app.patch('/api/companies/:id', async (req, res) => {
   ...
   app.post('/api/saved-jobs', async(req, res) => {
     try {
       ...
       if (!userId || !jobId) return res.status(400).send({error: 'userId and jobId are required'});
       const query = { userId, jobId };
       const existing = await savedJobsCollection.findOne(query);
       if (existing) {
         await savedJobsCollection.deleteOne(query);
         res.send({ saved: false });
       } else {
         await savedJobsCollection.insertOne({ userId, jobId, savedAt: new Date() });
         res.send({ saved: true });
       }
     } catch (error) {
       res.status(500).json({ error: error.message });
   ...
   app.get('/api/saved-jobs/:userId', async(req, res) => {
     try {
       ...
       const jobIds = savedJobs.map(sj => new ObjectId(sj.jobId));
       const jobs = await jobCollection.find({ _id: { $in: jobIds } }).toArray();
       const result = jobs.map(job => ({ ...job, isSaved: true }));
       res.send(result);
     } catch (error) {
       res.status(500).json({ error: error.message });
     }
